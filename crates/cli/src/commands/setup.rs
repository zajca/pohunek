...
# Linear CLI binary (required only for pohunek-launch-issue)
linear_cli=linear
...
Please implement the change end to end, then summarize what you did.
";
...
pub(crate) fn run_sway(
    ...
    let result = install_sway(paths, print, keybind)?;
    if json {
        print!("{}", crate::commands::render_json(&result)?);
    } else {
...
pub(crate) fn run_all(paths: &Paths, json: bool) -> Result<(), CliError> {
    ...
    let sway = install_sway(paths, false, DEFAULT_SWAY_KEYBIND)?;
...
/// Build the sway drop-in snippet binding `keybind` to the launcher at
/// `launcher`. Kept pure (no I/O) so it is directly unit-testable.
fn sway_snippet(launcher: &Path, keybind: &str) -> String {
    format!(
...
fn install_sway(paths: &Paths, print: bool, keybind: &str) -> Result<SwayResult, CliError> {
    let launcher = paths.launcher_bin_dir().join("pohunek-rofi");
    let snippet = sway_snippet(&launcher, keybind);
...
fn next_steps(paths: &Paths) -> Vec<String> {
    ...
    vec![
        ...
        "Reload sway (swaymsg reload), then press the keybind to open the launcher.".to_owned(),
    ]
}
...
mod tests {
    ...
    #[test]
    fn install_scripts_writes_all_five_executable() {
        let tp = temp_paths();
    ...
    #[test]
    fn install_sway_writes_dropin_with_keybind_and_launcher_path() {
        let tp = temp_paths();
        let keybind = "$mod+x";
        let result = install_sway(&tp.paths, false, keybind).expect("install sway");
        ...
        assert!(
            ...
            "drop-in must reference the absolute launcher path: {contents}"
        );
    ...
    fn install_sway_reports_include_present_when_main_config_references_dropin() {
        ...
        let result = install_sway(&tp.paths, false, DEFAULT_SWAY_KEYBIND).expect("install sway");
        assert!(
    ...
    #[test]
    fn sway_snippet_contains_bindsym_and_launcher() {
        let launcher = Path::new("/home/u/.local/share/pohunek/bin/pohunek-rofi");
        let snippet = sway_snippet(launcher, DEFAULT_SWAY_KEYBIND);
        ...
        assert!(
            ...
            "snippet must reference the launcher: {snippet}"
        );
    }
    ...
    fn install_sway_print_mode_touches_no_filesystem() {
        let tp = temp_paths();
        let result = install_sway(&tp.paths, true, DEFAULT_SWAY_KEYBIND).expect("print sway");
        assert!(result.printed);
