...
/// Semantic-review format version. Version 5 commits the account-linking
/// authority surface: each principal's account-link generation, every linked
/// identity's link provenance, and each link transaction's channel, state,
/// revision, source-authentication binding, and possession commitment.
...
async fn invalidate_old_generation(
    ...
    for statement in [
        ...
        "UPDATE browser_logins SET consumed_at=clock_timestamp(),outcome='cancelled' WHERE consumed_at IS NULL",
        "UPDATE device_logins SET consumed_at=clock_timestamp(),outcome='cancelled',poll_lease_until=NULL WHERE consumed_at IS NULL",
        "UPDATE account_link_transactions SET state='cancelled',completed_at=clock_timestamp(),revision=revision+1 WHERE state='pending'",
        "UPDATE evidence_challenges SET consumed_at=clock_timestamp(),consumption_state='invalidated' WHERE consumed_at IS NULL AND consumption_state='unused'",
...
async fn semantic_manifest(
    ...
    const QUERIES: &[(ManifestKind, &[&str], &str)] = &[
        (ManifestKind::Principal, &["id", "kind", "state", "generation", "account_link_generation"], "SELECT id::text,kind::text,state::text,generation::text,account_link_generation::text FROM principals ORDER BY id"),
        (ManifestKind::OidcIdentity, &["identity_id", "issuer", "subject", "principal_id", "link_generation", "linked_via_link_id", "removed_via_link_id", "removed_at"], "SELECT identity_id::text,issuer::text,subject::text,principal_id::text,link_generation::text,linked_via_link_id::text,removed_via_link_id::text,removed_at::text FROM oidc_identities ORDER BY identity_id"),
        ...
        (ManifestKind::AccountLink, &["link_id", "principal_id", "source_identity_id", "channel", "state", "revision", "linked_identity_id", "source_authentication_id", "source_authentication_generation", "issuer", "client_id", "audience", "digest_key_id", "account_link_generation", "recovery_generation", "created_at", "expires_at", "completed_at", "possession_digest_commitment"], "SELECT link_id::text,principal_id::text,source_identity_id::text,channel::text,state::text,revision::text,linked_identity_id::text,source_authentication_id::text,source_authentication_generation::text,issuer::text,client_id::text,audience::text,digest_key_id::text,account_link_generation::text,recovery_generation::text,created_at::text,expires_at::text,completed_at::text,encode(sha256(convert_to('pohunek-recovery-manifest-v5/account_link_transactions/possession_digest/','UTF8') || uuid_send(link_id) || possession_digest),'hex') FROM account_link_transactions ORDER BY link_id"),
