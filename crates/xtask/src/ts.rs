...
const RELAY_GENERATED_DIR: &str = "web/shared/src/relay-generated";
...
pub(crate) fn generate(root: &Path) -> Result<TsSummary, XtaskError> {
    let paths = TsPaths {
        generated: root.join(GENERATED_DIR),
        relay: root.join(RELAY_GENERATED_DIR),
        fixtures: root.join(FIXTURE_DIR),
    };
...
fn regenerate(root: &Path, paths: &TsPaths) -> Result<TsSummary, XtaskError> {
    remove_dir_all_if_exists(&paths.generated)?;
    remove_dir_all_if_exists(&paths.fixtures)?;
    create_dir_all(&paths.generated)?;
    create_dir_all(&paths.fixtures)?;

    let export_dir = absolute_path(&paths.generated)?;
    run_ts_rs_export(root, &export_dir, "pohunek-protocol")?;
    rewrite_ts_rs_headers(&paths.generated)?;

    let type_names = exported_type_names(&paths.generated)?;
    emit_index(&paths.generated, &type_names)?;
    emit_methods(&paths.generated)?;
    emit_constants(&paths.generated)?;
    emit_events(&paths.generated)?;
    emit_fixtures(&paths.fixtures)?;

    generate_relay(root, &paths.relay)?;

    Ok(TsSummary {
        generated_files: count_files(&paths.generated)? + count_files(&paths.relay)?,
        fixture_files: count_files(&paths.fixtures)?,
    })
}

fn run_ts_rs_export(root: &Path, export_dir: &Path, package: &str) -> Result<(), XtaskError> {
...
fn generate_relay(root: &Path, dir: &Path) -> Result<(), XtaskError> {
    remove_dir_all_if_exists(dir)?;
    create_dir_all(dir)?;
    run_ts_rs_export(root, &absolute_path(dir)?, "pohunek-relay-protocol")?;
    rewrite_ts_rs_headers(dir)?;
    let type_names = exported_type_names(dir)?;
    emit_index(dir, &type_names)?;
    write_generated_file(
        &dir.join("constants.ts"),
        &format!(
            "export const API_VERSION = {} as const;\nexport const EVIDENCE_SCHEMA_VERSION = {} as const;\n",
            relay_protocol::API_VERSION,
            relay_protocol::EVIDENCE_SCHEMA_VERSION
        ),
    )
}
...
fn session_full() -> SessionInfo {
    ...
    SessionInfo {
        ...
        subagents: vec![SubagentInfo {
            ...
            started_at_ms: 1_788_825_600_000,
            updated_at_ms: 1_788_825_600_000,
            finished_at_ms: None,
        }],
