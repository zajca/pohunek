...
async fn load_host_notifications(
    ...
    for params in notification_seed_queries() {
        let params_value = match serde_json::to_value(&params) {
            ...
            Err(err) => {
                tracing::event!(
                    name: "gui.notification_seed.serialize.failed",
                    tracing::Level::WARN,
                    host_id = %host_id,
                    error = %err,
                    "skipping notification seed query with unserializable params"
                );
                first_error.get_or_insert_with(|| {
                    format!("notification.list params failed to serialize: {err}")
                });
                continue;
            }
        };
        match request_json::<NotificationListResult>(
            client,
            "gui-notification-list",
            method::NOTIFICATION_LIST,
            params_value,
