...
model-free `cargo xtask hermes compatibility` gate exercises the pinned CLI and
plugin surface: list/enable/disable, supported tool/skill/hook registration,
profile/home target resolution, and Hermes integration
install/status/doctor/uninstall. It must use an isolated profile and must never
start a model turn, access an operator profile or `state.db`, read credentials,
or download a runtime. A missing pinned executable is a failure, never a green
skip. Release-archive verification additionally uses
...
explicitly and are never regenerated by CI. `cargo xtask hermes compatibility`
is expected to fail while any checked-in golden remains `pending`; do not report
that gate green until all required captures or a legitimate alternate-TUI
`unsupported` diagnosis are committed. The executable path must be absolute.
